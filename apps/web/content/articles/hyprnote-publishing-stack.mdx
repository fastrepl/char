...
Every CMS we evaluated pushed us away from the way we actually work.

We're a technical team. We write in our IDEs. We think in Git. We move fast. We rewrite things without ceremony. We treat documentation and blog posts as part of the product, not accessories to it.

When we looked at the options — WYSIWYG editors, headless CMS platforms, Git-based CMS layers, hosted docs systems — every option either slowed us down or boxed us in.

We did the simplest thing: we built our publishing system the same way we build Char.
...
- OG image generation that works
...
We don't need a CMS workflow because GitHub already solved that problem at scale:
...
It's not "content management." It's engineering.

Engineering workflows are better than every CMS workflow on the market.
...
It takes seconds to add an image. The image will still exist in 10 years.

Try that with a SaaS CMS.

## 5. Custom MDX Components: Our Secret Weapon

When content becomes code, every document becomes a canvas. We built a small but powerful set of MDX components:
...
The UI of our content is dictated by design, not by a CMS.

Because it's MDX, these components can become intelligent later: interactive, stateful, contextual, personalized, AI-enhanced.
...
We settled on a layered approach:
...
Our publishing system is built entirely on primitives that have survived 20+ years of architectural changes.

No matter what happens to the FE ecosystem next — Turbo, SSR, islands, React Server Components, Deno, Bun, something new — content in plain text will always be readable, migratable, and alive.
...
Every new system is a maintenance burden. We keep our stack simple.
