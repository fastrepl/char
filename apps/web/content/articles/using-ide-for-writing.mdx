...
At Char everyone writes and everyone codes. There's no sharp line between the two. Writing lives next to version control, next to the app logic, next to everything else we ship. When you treat writing as something that belongs in the same world as code, a lot of decisions get simpler.
...
WYSIWYG editors promise ease. Click here, bold there, drag an image, done. But once you try to do anything slightly unusual, they fall apart.
...
This brings us to the real reason writing in an IDE works so well: version control. Every blog post, every line, every word is tracked. Reviewed. Diffed. Discussed.
...
You never have to worry that your CMS broke something silently. You never have to trust a black box.
...
You can get the same convenience without giving up control.
