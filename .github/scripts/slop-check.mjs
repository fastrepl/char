...
const issueSchema = z.object({
  issues: z.array(
    z.object({
      ...
      original: z.string().describe("The exact original text that triggers the flag"),
      suggestion: z.string().describe("Rewritten text that sounds human-written"),
      reason: z.string().describe("Which AI writing pattern this matches and why it reads as LLM output"),
      category: z.enum([
        "slop-phrase",
        "structural-cliche",
        "rhythm-pattern",
      ...
      severity: z.enum(["high", "medium", "low"]).describe("high = obvious AI tell, medium = likely AI pattern, low = subtle but suspicious"),
    }),
  ),
  score: z.object({
    directness: z.number().min(1).max(10).describe("Statements or announcements?"),
  ...
  summary: z.string().describe("Overall assessment: how much does this read like AI-generated content, and what are the dominant patterns?"),
});
  ...
  for (const result of results) {
    ...
    if (result.error) {
      ...
      if (feedback.issues.length === 0) {
        ...
        const highCount = feedback.issues.filter((i) => i.severity === "high").length;
        const medCount = feedback.issues.filter((i) => i.severity === "medium").length;
        const lowCount = feedback.issues.filter((i) => i.severity === "low").length;
